...
FLIGHTS_DB = {
    ...
    ("Hà Nội", "Phú Quốc"): [
        ...
        {"airline": "Vietnam Airlines", "departure": "10:00", "arrival": "12:15", "price": 1_350_000, "class": "economy"},
...
HOTELS_DB = {
    "Đà Nẵng": [
        {"name": "Sala Danang Beach", "stars": 4, "price_per_night": 1_200_000, "area": "Mỹ Khê", "rating": 4.5},
        {"name": "Fivitel Danang", "stars": 3, "price_per_night": 650_000, "area": "Sơn Trà", "rating": 4.3},
        {"name": "Christina's Homestay", "stars": 2, "price_per_night": 350_000, "area": "An Thượng", "rating": 4.7},
        {"name": "Mường Thanh Luxury", "stars": 5, "price_per_night": 1_800_000, "area": "Mỹ Khê", "rating": 4.1},
...
def search_flights(origin: str, destination: str) -> str:
    ...
    result = f"Danh sách chuyến bay tuyến {origin} - {destination}:\n"
    for f in flights:
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    result = f"Khách sạn tại {city} (Giá <= {format_vnd(max_price_per_night)}):\n"
    for h in filtered_hotels:
        price_str = format_vnd(h['price_per_night'])
        result += f"- {h['name']} ({h['stars']} sao) | Khu vực: {h['area']} | Rating: {h['rating']} | Giá: {price_str}/đêm\n"
    return result
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        return "Lỗi định dạng expenses. Vui lòng nhập đúng định dạng 'tên_khoản: số_tiền, tên_khoản2: số_tiền'."
    ...
    if remaining >= 0:
        ...
        result += f"Vượt ngân sách {format_vnd(abs(remaining))} đồng! Cần điều chỉnh."
